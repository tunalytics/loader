SELECT tr.name as track_name, tr.length as song_length,  ar.name as artist_area, 
g.name as artist_gender,a.sort_name as artist_name, 
rec.name as recording_name,reca.end_date_year as recording_year from musicbrainz.track tr 
...
JOIN recording_alias_type rat ON reca.type = rat.id 
WHERE rat.id=1 and aat.id=1 LIMIT 1000;
